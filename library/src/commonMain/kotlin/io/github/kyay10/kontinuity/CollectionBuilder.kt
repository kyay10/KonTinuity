...
public open class ListBuilder<T> internal constructor(private val reader: ForkReader<MutableList<T>>) : MutableList<T> {
  ...
  public val readOnly: List<T>
    get() = reader.unsafeValue
...
public suspend fun <T> buildListLocally(body: suspend ListBuilder<T>.() -> Unit): PersistentList<T> = runListBuilder {
  body()
  readOnly.toPersistentList()
}

public suspend fun <T, R> runQueue(body: suspend Queue<T>.() -> R): R = runListBuilder front@{
  runListBuilder back@{ body(QueueAsTwoStacks(this@front, this@back)) }
}
...
public fun Queue<*>.isEmpty(): Boolean = size == 0

public fun Queue<*>.isNotEmpty(): Boolean = !isEmpty()

public inline fun <T> Queue<T>.dequeueAll(block: (T) -> Unit) {
  while (isNotEmpty()) block(dequeue())
}

private class QueueAsTwoStacks<T>(private val front: MutableList<T>, private val back: MutableList<T>) : Queue<T> {
  override val size: Int
  ...
  override fun dequeue(): T {
    if (front.isEmpty()) front.addAll(back.asReversed()).also { back.clear() }
...
public class MapBuilder<K, V> internal constructor(private val reader: ForkReader<MutableMap<K, V>>) :
  MutableMap<K, V> {
  public val map: MutableMap<K, V>
    get() = reader.value

  public val readOnly: Map<K, V>
    get() = reader.unsafeValue
