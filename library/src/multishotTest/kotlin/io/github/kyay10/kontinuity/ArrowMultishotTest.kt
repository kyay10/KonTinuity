...
class ArrowMultishotTest {
  @Test fun supportsMultishot() = runTestCC { handle<Int> { use { it(1) + it(2) } + 1 } shouldEq 5 }
  ...
  fun multishotResetShift() = runTestCC {
    "a" + handle { "b" + use { f -> "1" + f(f("c")) } } shouldEq "a1bbc"
    "a" + handle { "b" + use { f -> "1" + f(f("c")) } } shouldEq "a1bbc"
  }
  ...
  fun multishotMoreThanTwice() = runTestCC {
    handle<List<Pair<Pair<Int, Int>, Int>>> {
      ...
      listOf(i to j to k)
    } shouldEq listOf(10, 20).flatMap { i -> listOf(15, 25).flatMap { j -> listOf(17, 27).map { k -> i to j to k } } }
  }
  ...
  fun multishotMoreThanTwiceAndWithMoreMultishotInvocations() = runTestCC {
    handle<List<Pair<Pair<Int, Int>, Int>>> {
      ...
      listOf(10, 20, 30, 40, 50).flatMap { i ->
        listOf(15, 25, 35, 45, 55).flatMap { j -> listOf(17, 27, 37, 47, 57).map { k -> i to j to k } }
      }
  }
