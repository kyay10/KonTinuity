...
class TicTacToeTest {
  ...
  fun aiPrimeTest() =
    runTestCC(timeout = 10.minutes) {
      val _ = game(Mark.X, aiPrime, Mark.O, aiPrime)
    }

  @Ignore
  @Test
  fun aiTest() = runTestCC {
    val _ = game(Mark.X, ai, Mark.O, ai)
  }
}
...
enum class Move {
  ...
  operator fun invoke(loc: Location): Location =
    when (this) {
...
fun extendLoc(board: Board, move: Move, mark: Mark, loc: Location) = extendLocImpl(board, move, mark, 0, loc, move(loc))
...
fun showBoard(board: Board): String {
  return (0 until n).joinToString("\n", postfix = "\n-----------------------\n") { i ->
    (0 until n).joinToString("") { j -> board[Location(i, j)]?.let { " $it" } ?: " ." }
  }
}
...
fun takeMove(player: Mark, loc: Location, game: Game): Game {
  ...
  return Game(winner = winner, moves = game.moves.remove(loc), board = updatedBoard)
}
...
suspend fun <T> List<T>.choose(): T {
  forEachIteratorless { if (flip()) return it }
  raise()
}
...
fun estimateState(player: Mark, game: Game): Int =
  when {
    ...
    suspend fun aiPrimeLimited(depthLimit: Int, breadthLimit: Int, player: Mark, game: Game): Pair<Int, Game> =
      when {
        game.winner != null || game.moves.isEmpty() -> estimateState(player, game) to game
          ...
          onceOrNull { firstMoveWins(player.other, game) }
            ?.let { loc ->
