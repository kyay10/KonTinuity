...
class ExactInfTest {
  ...
  fun `test flip with sharing`() = runTestCC {
    ...
    result shouldContainExactlyInAnyOrder listOf(Probable(0.5, Value.Leaf(true)), Probable(0.5, Value.Leaf(false)))
  }

  @Test
  fun `test flip without sharing`() = runTestCC {
    val result = exactReify { flip() && flip() }
    result shouldContainExactlyInAnyOrder listOf(Probable(0.25, Value.Leaf(true)), Probable(0.75, Value.Leaf(false)))
  }
  ...
  fun `test grass model`() = runTestCC {
    ...
    result shouldContainExactlyInAnyOrder
      listOf(Probable(0.2838, Value.Leaf(true)), Probable(0.321999999999999897, Value.Leaf(false)))
  }

  @Test
  fun `test uniform range`() = runTestCC {
    val result = exactReify { (1..8).uniformly() }
    result shouldContainExactlyInAnyOrder (1..8).map { Probable(0.125, Value.Leaf(it)) }
  }

  @Test
  fun `test geometric distribution`() = runTestCC {
    val result = reify { geometric(0.85) }.explore(maxDepth = 4)
    result.filter { it.value is Value.Leaf } shouldContainExactlyInAnyOrder
      listOf(
    ...
    result.mapNotNull { it.prob.takeIf { _ -> it.value is Value.Branch } } shouldContainExactlyInAnyOrder
      listOf(1.13906250000000092e-05, 6.45468750000000426e-05, 0.000430312500000000248)
  }

  @Test
  fun `test geometric bounded`() = runTestCC {
    val result = exactReify { geometric(0.85, endInclusive = 7) }
    result shouldContainExactlyInAnyOrder
      listOf(
  ...
  fun `test flips and`() = runTestCC {
    val result = exactReify {
      tailrec suspend fun loop(n: Int): Boolean {
        return if (n == 1) flip() else flip() && loop(n - 1)
      }
      loop(10)
    }
    result shouldContainExactlyInAnyOrder
      listOf(Probable(0.0009765625, Value.Leaf(true)), Probable(0.9990234375, Value.Leaf(false)))
  }
  ...
  fun `test flips xor incorrect`() = runTestCC {
    val result = exactReify {
      ...
      suspend fun loop(n: Int): Boolean = if (n == 1) flip() else variableElimination { loop(n - 1) } != flip()
      loop(10)
    }
    result shouldContainExactlyInAnyOrder listOf(Probable(0.5, Value.Leaf(true)), Probable(0.5, Value.Leaf(false)))
  }
    ...
    val result = exactReify {
      ...
      suspend fun loop(n: Int): Boolean =
        if (n == 1) flip()
          ...
          variableElimination { loop(n - 1) } != flip()
        }
      yieldToTrampoline()
      loop(10_000)
    }
